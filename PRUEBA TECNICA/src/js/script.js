...
closeModalBtnsNuevo.forEach((btn) => {
  ...
  scrollButton.addEventListener('click', () => {
    const targetPosition = bottom.getBoundingClientRect().top + window.pageYOffset;
    window.scrollTo({
      top: targetPosition,
      behavior: 'smooth'
    });
...
function agregarProducto(producto, tamanio, cantidad) {
  ...
  cantidadInput.addEventListener("change", function() {
    actualizarFila(this.parentNode.parentNode, producto, tamanio, cantidadInput.value);
  });
...
function enviarWhatsApp() {
  ...
  carrito.forEach(function(item) {
    ...
    let cantidad = item.querySelector("#cantidadDeProductos input[type='number']").value;  
  ...
  carritoModal.forEach(function(item) {
    ...
    comentario += item.querySelector("#comentario input[name='comentario']").value;
  if (!comentario) {
  ...
  if (!comentario) {
    alert("Es necesario agregar su direccion a la nota");
    return;
  }
  });
  ...
  if (!comentario) {
    alert("Por favor agregue su email a la nota");
    return;
  }
  });
//Agregar el total a la comanda de whatsapp
...
comer.addEventListener("change", function() {
if (comer.checked) {
tipoEntregaSeleccionado = "Estoy en el Restaurante - Mesa " + numeroMesa.value;
...
efectivo.addEventListener("click", function() {
  if (efectivo.checked) {
    formaDePagoSeleccionada = "Efectivo";
    formaDePagoSeleccionada = "Voy a pagar con un billete de " + concuanto.value;
  }
});
...
const numeroMesaInput = document.querySelector('#miModalInterno input[placeholder="Número de mesa"]');
...
tipoEntregaInputs.forEach(input => {
  input.addEventListener('change', () => {
    ...
    const tipoEntregaSeleccionada = document.querySelector('input[name="tipo-entrega"]:checked').value;
...
if (totalFinalComandaElement) 

// Escuchamos los cambios en los elementos correspondientes y actualizamos el resultado
document.getElementById('total3').addEventListener('DOMSubtreeModified', sumarTotalYPropina);
document.getElementById('propina').addEventListener('DOMSubtreeModified', sumarTotalYPropina);
document.getElementById('total').addEventListener('DOMSubtreeModified', sumarTotalYPropina);
